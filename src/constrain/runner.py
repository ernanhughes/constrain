...
from constrain.policy.custom_types import ThresholdProvider, Thresholds, PolicyDecision
...
def run(policy_id: int = 4, seed: int = 42, num_problems: int = None, threshold: float = None) -> str:
    ...
    # ==========================================================
    # SIGNAL DISCOVERY (optional, configurable)
    # ==========================================================

    if cfg.run_signal_discovery:  # New config flag
        try:
            ...
            if not results.get("skipped"):
                signal_service.persist_signal_report(
                    run_id=run_id,
                    results=results,
                    experiment_id=run.experiment_id,  # Link to experiment
                )
                
            logger.debug("Signal discovery completed for run: %s", run_id)
        except Exception as e:
            logger.exception("Signal discovery failed: %s", e)
...
def run_with_provider(
    policy_id: int,
    threshold_provider: ThresholdProvider,  # ← Inject provider directly
    seed: int = 42,
    num_problems: int = None,
) -> str:
    """
    Run experiment with injected threshold provider.
    Bypasses config-based threshold selection.
    """
    start_time = time.time()
    logger.debug("=" * 60)
    logger.debug("Starting run_with_provider: policy_id=%d, seed=%d", policy_id, seed)
    logger.debug("=" * 60)
    ...
    # ==========================================================
    # REGISTER RUN (use static thresholds for record-keeping)
    # ==========================================================
    # Note: We still log the thresholds used, even though they come from provider
    run_dto = RunDTO(
        run_id=run_id,
        model_name=cfg.model_name,
        initial_temperature=cfg.initial_temperature,
        num_problems=num_problems or cfg.num_problems,
        num_recursions=cfg.num_recursions,
        tau_soft=threshold_provider.get(cfg=cfg, memory=memory, run_id=run_id).tau_soft,
        tau_medium=threshold_provider.get(cfg=cfg, memory=memory, run_id=run_id).tau_medium,
        tau_hard=threshold_provider.get(cfg=cfg, memory=memory, run_id=run_id).tau_hard,
        policy_id=policy_id,
        task_type="gsm8k",
        start_time=start_time,
        status="running",
    ...
    # ----------------------------------------------------------
    # POLICY ENGINE WITH INJECTED PROVIDER  ← KEY CHANGE
    # ----------------------------------------------------------
    engine = PolicyEngine(
