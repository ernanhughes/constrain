...
class MetricsAnalyzer:
    ...
    def compute_per_problem_drift(self, run_id: str) -> pd.DataFrame:
        ...
        for pid in df['problem_id'].unique():
            ...
            final_energy = float(final_energy) if pd.notna(final_energy) else np.nan
            final_accuracy = float(final_accuracy) if pd.notna(final_accuracy) else np.nan
            final_phase = float(final_phase) if pd.notna(final_phase) else np.nan
